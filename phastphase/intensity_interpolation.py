...
def interpolation_operator(support_mask, missing_intensity_indices, autocorr_indices):
    P = jnp.zeros((jnp.size(autocorr_indices), len(missing_intensity_indices[0])), dtype=jnp.complex128)
    ...
    for col_index, intensity_index in enumerate(missing_index_list):
        P = P.at[:, col_index].set(ifft_vec(support_mask, intensity_index)[autocorr_indices])
    return P
...
def interpolate_intensities(far_field_intensities, #Total far-field pattern, with missing elements set to 0
    ...
    autocorr_indices, flat_autocorr_indices = calc_autocorr_indices_choice(near_field_support_mask, num_matrix_rows)
    b_complex = -calculate_residual(far_field_intensities, autocorr_indices)
    interpolation_matrix_complex = interpolation_operator(near_field_support_mask, missing_intensity_indices, flat_autocorr_indices)
...
def replace_negative_intensities(far_field_intensities,
    ...
    missing_intensities = interpolate_intensities(far_field_intensities, near_field_support_mask, missing_indices, num_matrix_rows,non_neg_least_squares)

    far_field_intensities = far_field_intensities.at[missing_indices].set(missing_intensities)
    return far_field_intensities

def interpolation_linear_operator(missing_intensities,
                                  full_intensities,
                                  missing_intensity_indices,
                                  autocorr_indices):
    full_intensities = full_intensities.at[missing_intensity_indices].set(missing_intensities, unique_indices=True)
...
def matrix_free_intensity_interpolation(far_field_intensities, #Total far-field pattern, with missing elements set to 0
                            near_field_support_mask, #Support mask, the total size should be the size of the far-field
                            missing_intensity_indices,#indices of the missing intensity measurements. Should of the form you get from jax.numpy.nonzero)
                            rtol:float = 1e-9,
                            atol:float = 1e-8): 
    autocorr_indices, flat_autocorr_indices = calc_autocorr_indices(near_field_support_mask)
    b_complex = -calculate_residual(far_field_intensities, autocorr_indices)
    b_real_imag = jnp.concat([jnp.real(b_complex), jnp.imag(b_complex)])
    full_intensities = jnp.zeros_like(far_field_intensities)
    non_flat_indices= jnp.unravel_index(missing_intensity_indices, jnp.shape(full_intensities))
    def interp_operator(x):
        return interpolation_linear_operator(x,full_intensities, non_flat_indices, autocorr_indices )
    dummy = jnp.zeros(jnp.size(missing_intensity_indices))
    linear_operator = lx.FunctionLinearOperator(interp_operator, jax.eval_shape(lambda: dummy))
    #solver = LSMR(rtol, atol)
    solver = lx.NormalCG(rtol, atol)
    out = lx.linear_solve(linear_operator, b_real_imag, solver)
    return out.value

def mat_free_replace_negative_intensities(far_field_intensities,
                                        near_field_support_mask,
                                        rtol:float = 1e-12,
                                        atol:float = 1e-12):
    index_mask = jnp.zeros_like(far_field_intensities)
    ...
    missing_intensities = matrix_free_intensity_interpolation(far_field_intensities, near_field_support_mask, missing_indices, rtol, atol)
    far_field_intensities = far_field_intensities.at[non_flat_indices].set(missing_intensities)
    return far_field_intensities
